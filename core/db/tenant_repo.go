...
// tenantSelectCols lists every column read by the scanTenant helper. Keep
// in lockstep with scanTenant's Scan target order.
const tenantSelectCols = `id, name, slug, COALESCE(domain, ''), COALESCE(issuer_url, ''), parent_id, settings, branding, created_at, updated_at`

func scanTenant(row pgx.Row) (*models.Tenant, error) {
	var t models.Tenant
	if err := row.Scan(
		&t.ID, &t.Name, &t.Slug, &t.Domain, &t.IssuerURL, &t.ParentID,
		&t.Settings, &t.Branding, &t.CreatedAt, &t.UpdatedAt,
	); err != nil {
		return nil, err
	}
	return &t, nil
...
func (r *tenantRepo) Create(ctx context.Context, tenant *models.Tenant) error {
	...
	_, err := r.pool.Exec(ctx, `
		INSERT INTO tenants (id, name, slug, domain, issuer_url, parent_id, settings, branding, created_at, updated_at)
		VALUES ($1, $2, $3, $4, NULLIF($5, ''), $6, $7, $8, $9, $10)`,
		tenant.ID, tenant.Name, tenant.Slug, tenant.Domain, tenant.IssuerURL,
		tenant.ParentID, tenant.Settings, tenant.Branding, tenant.CreatedAt, tenant.UpdatedAt,
	)
	if err != nil {
...
func (r *tenantRepo) GetByID(ctx context.Context, id uuid.UUID) (*models.Tenant, error) {
	t, err := scanTenant(r.pool.QueryRow(ctx, `SELECT `+tenantSelectCols+` FROM tenants WHERE id = $1`, id))
	if err != nil {
...
func (r *tenantRepo) GetBySlug(ctx context.Context, slug string) (*models.Tenant, error) {
	t, err := scanTenant(r.pool.QueryRow(ctx, `SELECT `+tenantSelectCols+` FROM tenants WHERE slug = $1`, slug))
	if err != nil {
		if err == pgx.ErrNoRows {
			return nil, models.ErrNotFound
		}
...
func (r *tenantRepo) GetByDomain(ctx context.Context, domain string) (*models.Tenant, error) {
	t, err := scanTenant(r.pool.QueryRow(ctx, `SELECT `+tenantSelectCols+` FROM tenants WHERE domain = $1`, domain))
	if err != nil {
		if err == pgx.ErrNoRows {
			return nil, models.ErrNotFound
		}
...
func (r *tenantRepo) Update(ctx context.Context, tenant *models.Tenant) error {
	...
	_, err := r.pool.Exec(ctx, `
		UPDATE tenants SET name = $1, slug = $2, domain = $3, issuer_url = NULLIF($4, ''),
			parent_id = $5, settings = $6, branding = $7, updated_at = $8
		WHERE id = $9`,
		tenant.Name, tenant.Slug, tenant.Domain, tenant.IssuerURL,
		tenant.ParentID, tenant.Settings, tenant.Branding, tenant.UpdatedAt, tenant.ID,
	)
	if err != nil {
...
func (r *tenantRepo) List(ctx context.Context, params models.PaginationParams) (*models.PaginatedResult[models.Tenant], error) {
	...
	rows, err := r.pool.Query(ctx, `SELECT `+tenantSelectCols+` FROM tenants ORDER BY created_at DESC LIMIT $1 OFFSET $2`, params.PerPage, offset)
	if err != nil {
